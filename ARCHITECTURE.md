...
| **git-core** | Dedicated service owning bare repositories on local disk. Exposes a narrow internal API (create/delete/move repo, info-refs, upload-pack/receive-pack streaming, tree/blob reads, branch/tag ops). Runs `git` plumbing as subprocesses (never parses objects itself). **Status:** implemented today as an in-process engine (`server/src/storage/repository.ts`) producing standard loose-object repositories with lockfile+CAS ref updates; no subprocess is spawned and user input never reaches a command line (see STORAGE.md §10). | Gitaly |
